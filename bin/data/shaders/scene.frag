...
//        float map_scale = map(color.r, 0.f, 1.f, 1.f, 1.2f );
//
//        texcoord -= vec2(0.5f);
...
//        texcoord += vec2(0.5f);
        ...
        if ( s > (n_steps-1) && (n_steps-1) == 11 ) { img_color = texture(img11, texcoord * tex_resolution); }

        if ( s == 10 ) { img_color = texture(img10, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 10 ) { img_color = texture(img10, texcoord * tex_resolution); }

        if ( s == 9 ) { img_color = texture(img9, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 9 ) { img_color = texture(img9, texcoord * tex_resolution); }

        if ( s == 8 ) { img_color = texture(img8, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 8 ) { img_color = texture(img8, texcoord * tex_resolution); }

        if ( s == 7 ) { img_color = texture(img7, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 7 ) { img_color = texture(img7, texcoord * tex_resolution); }

        if ( s == 6 ) { img_color = texture(img6, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 6 ) { img_color = texture(img6, texcoord * tex_resolution); }
        
        if ( s == 5 ) { img_color = texture(img5, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 5 ) { img_color = texture(img5, texcoord * tex_resolution); }

        if ( s == 4 ) { img_color = texture(img4, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 4 ) { img_color = texture(img4, texcoord * tex_resolution); }

        if ( s == 3 ) { img_color = texture(img3, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 3 ) { img_color = texture(img3, texcoord * tex_resolution); }

        if ( s == 2 ) { img_color = texture(img2, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 2 ) { img_color = texture(img2, texcoord * tex_resolution); }

        if ( s == 1 ) { img_color = texture(img1, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 1 ) { img_color = texture(img1, texcoord * tex_resolution); }
        
        if ( s == 0 ) { img_color = texture(img0, texcoord * tex_resolution); }
        if ( s > (n_steps-1) && (n_steps-1) == 0 ) { img_color = texture(img0, texcoord * tex_resolution); }
